...
def create_cuboid(cub, data):
    ...
    mat.base_color = cuboid_colors[classTitle] + [0.5]  # RGBA, adjust A for transparency

    return cuboid_3d, mat
...
# geom = [{'name': 'cloud', 'geometry': cloud}, {'name': 'axes', 'geometry': axes}]
geom = [{'name': 'cloud', 'geometry': cloud}]
for i, (c, m) in enumerate(cuboids):
    geom.append({'name': f'cuboid{i}', 'geometry': c, 'material': m})

o3d.visualization.draw(geom, show_skybox=False)
